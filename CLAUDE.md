...
- **Releases:** when `main` has accumulated enough change (the owner's call), cut a release branch named **`release-X.Y.Z`** (e.g. `release-1.0.0`) from `main`, then bump `main`'s `VERSION` to the next dev number. Deploy by pushing a **`vX.Y.Z` tag** on the release branch — the tag (not the branch) triggers the deploy. A release branch **stays rooted at the commit it was cut from** — never rebase it or move its base.
