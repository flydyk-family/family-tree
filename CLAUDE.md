...
A family-tree viewer: a **.NET 10 JSON-backed API** plus a **Vue 3 SPA** that renders the family as an SVG "oak" — a vertical time axis, whole-tree pan/zoom, medallion person cards (portrait + name + birth–death years), and a glass detail popup. Two switchable **themes**: the default **Film** (period-accurate photo cards — cabinet card / silver-gelatin / colour film frame by birth year — on a brushed-metal backdrop, with curved family connectors in red rope texture — each couple's spouse curves converge at a single joint that fans one curve out to each child, minimising line crossings) and **Classic** (gilt-frame oval medallions on warm parchment). Public data is served from a seed `family.json` via an in-memory merged snapshot (10-min TTL, refreshed on editor save); in deployment the seed is sourced from **Google Cloud Storage** (`FamilyData:Source=gs://…`) and is swappable without a redeploy — local dev/tests read the committed file. Text is localized (ru primary / be / en). Authenticated editors (Google sign-in, allow-list controlled) can update biography text and upload portrait / gallery photos in-app; edits and photos persist durably in **Google Firestore** and **Cloudflare R2** in deployment (in-memory / local filesystem locally); uploaded photos are never written back to the `family.json` seed — the `scripts/upload-media.mjs` bulk path remains for seed media. The app can serve **multiple family trees** from an optional `families.json` registry (one seed per family) — opt in via `FamilyData:Registry`, unset in production today; when 2+ families are registered, a **switcher** appears in Settings and a person's card can carry a button ("{name} family tree" / "Family they joined: {name}") to their record in another family tree, jumping there and centering the camera on arrival. The app bar ships a **Sign in with Google** control (a fixed account slot — rightmost on the desktop bar, top-right on mobile — that collapses to an initials-avatar account menu once signed in) backed by Google Identity Services; signing in posts the ID token to `POST /api/auth/session`, the resulting session cookie is used for all auth calls, and `GET /api/auth/me` is called on load to hydrate the signed-in identity and `canEdit` flag. The **biography editing UI** is shipped: signed-in editors edit the localized biography inline in the bigger-view popup (tabbed ru/be/en editor with resilient save) via `PUT /api/people/{id}/biography`. The **photo manager** is also shipped: a **unified photo grid** in the bigger-view popup lets editors upload photos (`POST /api/people/{id}/photos`, `multipart/form-data`, `role=portrait|gallery`), set any image as the portrait, remove photos, and hide seed media (`DELETE /api/people/{id}/photos/seed/{role}`); media is capped at **5 items per person** (portrait + gallery + the living-portrait video, which shows as a grid tile), uploads are re-encoded to WebP (**downscale-only**, EXIF stripped) and stored in R2 at content-hashed `uploads/{personId}/{hash}.webp` keys, and the same grid is visible read-only to all visitors.
...
- `src/frontend/` — **Vue 3 + TypeScript + Vite**. Pinia stores, Vue Router (`/person/:slug` friendly deep link — name + birth year + id; every view is also routed under `/f/:familyId/…` for non-default family trees, see `router/familyRoutes.ts`), vue-i18n, SCSS design tokens (`src/styles/tokens.scss`). A **custom layout engine** (`src/layout/treeLayout.ts` + `timeScale.ts` + `familyRouting.ts`) computes positions; Vue owns the SVG (`OakTree.vue`, `PersonMedallion.vue`, `YearAxis.vue`, `PersonPopup.vue`, `FamilyConnector.vue`). The dev server proxies `/api` and `/assets` to the API.
...
- shell: not applicable (no *.sh scripts in the repo)
