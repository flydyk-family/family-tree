...
.stats {
  ...
  &__title { font-family: var(--font-display); font-weight: 600; font-size: 17px; letter-spacing: 1px; text-align: center; margin: 2px 0 4px; }
  &__rule { height: 1px; background: linear-gradient(90deg, transparent, var(--gilt), transparent); margin: 7px 2px 12px; }
  &__row { display: flex; justify-content: space-between; align-items: baseline; padding: 9px 2px; border-bottom: 1px dashed rgba(111, 90, 60, 0.22); &:last-child { border-bottom: none; } }
  &__label { font-family: var(--font-body); font-size: 15px; color: var(--ink); }
  &__value { font-family: var(--font-display); font-weight: 600; font-size: 17px; color: var(--umber); }
}
</style>
