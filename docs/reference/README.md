...
> - **Members tab — shipped, read-only, redesigned "Family Chronicle" look.** `/members/:slug?` opens a searchable master-detail roster (Generation/Surname/Place filters, place-aware search) + an ornate per-person dossier + a three-column immediate-family cluster; the app-bar tab is enabled and the app bar itself has a heraldic crest + per-tab icons. Fields, biography, residences, and the photo grid are **display-only** — there is no in-app editor UI yet, though the redesign is layout-ready for it (the backend `PUT /api/people/{id}/profile` ships dormant). See [features/search-and-navigation.md](features/search-and-navigation.md#members-page-readonly-membersslug). **Timeline** tab — still rendered but **disabled** ("Coming soon"); no route, no view.
