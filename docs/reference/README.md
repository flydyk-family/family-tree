...
> - **Members tab — shipped; scalar fields, biography, and residences are all editable.** `/members/:slug?` opens a searchable master-detail roster + per-person dossier + immediate-family cluster; the app-bar tab is enabled. A signed-in editor can edit a person's **scalar fields** (given/surname/maiden name ×3 locales, sex, birth/death year, vocation) inline via an **Edit details** editor that posts to `PUT /api/people/{id}/profile` — only changed fields override the seed, with a per-field reset-to-seed. **Biography** has its own editor. **Residences** also have their own editor (cut 1c) with an interactive Google Maps picker (city search + draggable pin; degrades to manual lat/lng entry without `VITE_GOOGLE_MAPS_API_KEY`) and a whole-list `residences` override on the same `PUT` endpoint; the read-only view (visitors, and editors when not editing) shows a plain outbound "open in Google Maps" link, never an embedded map. Birth/death **place** editing and the photo grid (intentionally read-only here) remain unbuilt/deferred; add/remove people is cut 2. See [features/search-and-navigation.md](features/search-and-navigation.md#members-page-readonly-membersslug). **Timeline** tab — still rendered but **disabled** ("Coming soon"); no route, no view.
> - **Auth / editing — sign-in UI and biography editor both shipped.** Google sign-in + server-side session + editor-gated biography editing exist at the API level and are covered by integration tests. The **frontend sign-in UI** is shipped — the app bar has a **Sign in with Google** control; see [features/app-shell-and-localization.md](features/app-shell-and-localization.md#sign-in--sign-out). The **in-app biography editor** is also shipped — signed-in editors can edit the localized biography inline in the bigger-view popup via `PUT /api/people/{id}/biography`; see [features/person-details.md](features/person-details.md#editing-a-biography-signed-in-editors). Sessions and biography edits are **in-memory only** locally and reset on API restart; Firestore persistence is enabled in deployment when `Firestore:ProjectId` is configured.
> - **Portrait media** — served from Cloudflare R2; without it the UI shows **initials fallback** (see [features/person-details.md](features/person-details.md#media--living-portraits)). **In-app photo upload** is also shipped — editors can upload portraits and gallery photos; the same photo grid is visible read-only to all visitors (see [features/person-details.md](features/person-details.md#photo-grid) and [features/backend-api.md](features/backend-api.md#authentication--editor-endpoints)).
...
> - **Multiple family trees — switching is live in the app, visible only when the registry lists 2+ families.** The backend is in place: the optional `families.json` registry, per-family snapshots, family-scoped storage keys, `familyLinks` on person payloads (always `[]` unless a seed sets them), and the family-scoped API routes (`GET /api/families`, `GET /api/families/{familyId}/graph`, `/api/families/{familyId}/people/…`, `degradedFamilies` in `/health`). The SPA now has a **Family tree switcher** in the Settings popover/mobile sheet and every route exists both unprefixed (default family) and under `/f/:familyId` — see [features/app-shell-and-localization.md](features/app-shell-and-localization.md#family-tree-switcher-familyswitchervue) and [features/search-and-navigation.md](features/search-and-navigation.md#routing--deep-links-routerindexts-routerfamilyroutests-viewstreeviewvue). With no registry configured (**production today**), `/api/families` lists only the single `default` family, so `familiesStore.hasMultiple` is false and **the switcher renders nothing** — every route still behaves exactly as before. **Also live:** a person's card carries a button per registered `familyLinks` entry, which jumps to that person's record in their other family tree and glides the camera there — see [features/person-details.md](features/person-details.md#persondetail-content-persondetailvue). See [roadmap.md](roadmap.md) and [features/backend-api.md](features/backend-api.md).
...
A family viewer backed by a localized graph of **people** and **unions** from a seed [`family.json`](../../src/backend/FamilyTree.Api/Data/family.json). The frontend renders it as a vertical (or horizontal) **oak**: a time axis, whole-tree pan/zoom, **medallion** person cards, a glass **person detail** surface, live **search**, a first-visit **Chronicle** landing page, and **ru / be / en** localization. Two switchable **themes** are available: **Classic** (gilt-frame oval medallions on a warm parchment canvas) and **Film** (period-accurate photo-card medallions on a muted studio-grey canvas — see [features/app-shell-and-localization.md](features/app-shell-and-localization.md#theme-toggle) and [features/oak-tree.md](features/oak-tree.md#eighties-film-theme-medallions)). Authenticated editors (Google sign-in, allow-list controlled) can update biography text and manage photos via the API and the in-app editor and **photo grid** (`PersonPhotos`); see [features/backend-api.md](features/backend-api.md#authentication--editor-endpoints) for the backend contract, [features/app-shell-and-localization.md](features/app-shell-and-localization.md#sign-in--sign-out) for the sign-in flow, and [features/person-details.md](features/person-details.md#editing-a-biography-signed-in-editors) for the editor UI. **The same photo grid is visible read-only to all visitors** (and renders nothing when a person has no photos).
