...
> - **Multiple family trees — backend API only, not live in the app.** The backend is in place: the optional `families.json` registry, per-family snapshots, family-scoped storage keys, `familyLinks` on person payloads (always `[]` unless a seed sets them), and the family-scoped API routes (`GET /api/families`, `GET /api/families/{familyId}/graph`, `/api/families/{familyId}/people/…`, `degradedFamilies` in `/health`). With no registry configured (production today), `/api/families` lists the single `default` family and every existing route behaves as before. The SPA never calls a family-scoped route: there is **no** family switcher and **no** member-card links to other trees yet. See [roadmap.md](roadmap.md) and [features/backend-api.md](features/backend-api.md).
