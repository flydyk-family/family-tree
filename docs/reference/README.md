...
> - **Multiple family trees — backend foundation only, not live.** Only the backend foundation exists: the optional `families.json` registry, per-family snapshots, family-scoped storage keys, and `familyLinks` on person payloads (always `[]` unless a seed sets them). With no registry configured (production today), the API serves the single family exactly as before. There are **no** family-scoped API routes, **no** family switcher, and **no** member-card links to other trees yet. See [roadmap.md](roadmap.md) and [features/backend-api.md](features/backend-api.md).
