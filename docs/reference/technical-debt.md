...
- **Custom domain + ECH.** Production serves from the apex custom domain `https://perovsky.family`; the Pages URL `https://family-tree-4fl.pages.dev` is the deploy target and a mirror (plain `family-tree.pages.dev` is someone else's). `*.pages.dev` is **blocked in Belarus/Russia** (blanket `pages.dev` filtering + DPI dropping Cloudflare's default-on ECH), so reachability there depends on the custom domain having **ECH disabled** on its Cloudflare zone — an out-of-band setting not captured in repo config; if the zone is recreated or ECH is re-enabled, BY/RU access breaks again. Runbook: [`docs/ci-cd/custom-domain-and-ech.md`](../../docs/ci-cd/custom-domain-and-ech.md).
