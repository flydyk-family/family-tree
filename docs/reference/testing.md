...
### Frontend tests (100 spec files, 879 cases)
...
- **Composables:** `useSearchMatches` (substring, name order, **maiden-name match**, cursor wrap), `useFamilyStats`, `useMediaQuery`, `useRelatives` (parents/siblings/spouses/children derivation, half-siblings, no relations, unknown person id), `profileDraft` (**pure override-payload builder: `seedDraft` independence; untouched field stays null; existing untouched override preserved; changed scalar/cleared year; reverted field → null; per-locale name overlay preserving other overridden/seed-inherited locales; all-blank name collapses to null**).
