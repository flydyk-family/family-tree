...
### Backend unit tests ([`tests/unit/FamilyTree.UnitTests`](../../tests/unit/FamilyTree.UnitTests), 168 cases)
...
- **Validators / pipeline** — `GetPersonByIdQueryValidator` accepts `p-0001`, rejects `""`/`invalid`/`x-0001`; `ValidationBehavior` throws on invalid and calls next on valid. `UpdatePersonBiographyValidator` validates id format **and the 20,000-char-per-locale length cap** (at-limit passes, over-limit fails — primary and secondary locales). `UpdatePersonProfileValidatorTests`: id pattern; birth/death year range `[1000, 2100]`; birth ≤ death; a provided name object needs at least one non-blank locale; an unparseable `sex`/`vocation` string is **rejected** (not silently dropped), while a valid lowercase enum string passes; `null` fields (inherit-seed) always pass.
...
### Backend integration tests ([`tests/integration/FamilyTree.IntegrationTests`](../../tests/integration/FamilyTree.IntegrationTests), 58 cases)
...
### Frontend tests (97 spec files, 846 cases)
