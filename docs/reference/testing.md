...
### Backend unit tests ([`tests/unit/FamilyTree.UnitTests`](../../tests/unit/FamilyTree.UnitTests), 163 cases)
...
- **Validators / pipeline** — `GetPersonByIdQueryValidator` accepts `p-0001`, rejects `""`/`invalid`/`x-0001`; `ValidationBehavior` throws on invalid and calls next on valid. `UpdatePersonBiographyValidator` validates id format **and the 20,000-char-per-locale length cap** (at-limit passes, over-limit fails — primary and secondary locales). `UpdatePersonProfileValidatorTests`: id pattern; birth/death year range `[1000, 2100]`; birth ≤ death; a provided name object needs at least one non-blank locale; `null` fields (inherit-seed) always pass.
...
### Backend integration tests ([`tests/integration/FamilyTree.IntegrationTests`](../../tests/integration/FamilyTree.IntegrationTests), 55 cases)
...
- **Profile endpoints** (`PeopleProfileEndpointsTests`, 3 cases): `GET /api/people/{id}/profile` on an existing person → 200; `PUT` without a session → **401**; a signed-in editor's birth-year edit → 200, and the corrected year is visible on a follow-up `GET /api/family/graph` (the split-brain / snapshot-merge check).
...
### Frontend tests (95 spec files, 829 cases)
...
- **Composables:** `useSearchMatches` (substring, name order, **maiden-name match**, cursor wrap), `useFamilyStats`, `useMediaQuery`, `useRelatives` (parents/siblings/spouses/children derivation, half-siblings, no relations, unknown person id).
...
- **Members page:** `MembersIndex` (search filter incl. maiden name, sort by surname, empty state, selection highlight), `MemberFamilySheet` (relative groups render/hide, chip click emits selection), `TabNav` (Members tab now enabled and navigable, Timeline still disabled).
...
- **Members page** end-to-end selection flow (`MembersView`/`MemberDetail` — deep-link resolution, fetch loading/error states, Find-on-tree navigation) is not covered by automated tests, only its constituent pieces (`MembersIndex`, `MemberFamilySheet`, `useRelatives`).
- **`PUT /api/people/{id}/profile`** end-to-end from a real editor UI — the endpoint's backend contract is integration-tested, but there is no frontend caller yet to exercise (cut 1b).
