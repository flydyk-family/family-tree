...
Key components: `OakTree.vue`, `PersonMedallion.vue` (+ `components/medallion/`), `TimeRail.vue`; engine: `layout/treeLayout.ts`, `layout/timeScale.ts`, `layout/projection.ts`, `layout/focusBounds.ts`.
...
| Selected (`selected`) | `frame-selected.svg` (brighter gold) crossfades in (0.3 s) |
| Search match (`match`) | `frame-match.svg` (green-gold) crossfades in — **match wins over selected** |
...
The motion engine (`motion/`) is GSAP-based; every animation checks `prefers-reduced-motion` and snaps instantly when reduced.
...
- **VocationIcon:** inline SVG for `teacher`/`church`/`writer`/`office`/`other`; renders nothing for unknown vocations.
