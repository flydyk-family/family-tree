...
## Loading, error & family switching ([`stores/familyStore.ts`](../../../src/frontend/src/stores/familyStore.ts), [`router/familySync.ts`](../../../src/frontend/src/router/familySync.ts), [`views/TreeView.vue`](../../../src/frontend/src/views/TreeView.vue)) {#loading--error-states}

`familyStore` holds the graph for **one family at a time**; `familyStore.ensureFamily(familyId)` is the single entry point that shows it. [`installFamilySync`](../../../src/frontend/src/router/familySync.ts) calls it in the router's `afterEach`, keyed off [`activeFamilyId(route)`](search-and-navigation.md#routing--deep-links-routerindexts-routerfamilyroutests-viewstreeviewvue) — every successful navigation re-derives which family should be showing; `TreeView` also calls it on mount so a hard refresh onto a family route loads directly.

- **States:** `TreeView` shows a loading message (`status.loading`) while `familyStore.loading`, the oak layout once people/unions arrive, or an **error status** (`status.error`) plus a **"Back to the main tree"** link (`data-test="back-to-main-tree"`, `family.backToMain`) when `familyStore.error` is set — shown only while on a family-prefixed route (`activeFamilyId(route)` truthy), since the unprefixed default-family route has nowhere else to link to. The same error state and back link appear on `ChronicleView` and `MembersView` too — not only `TreeView` — whichever view the failed navigation lands on.
- **Unknown family:** `/f/<unregistered-id>` resolves as a route (the path pattern matches any id) but the graph fetch 404s, landing in the error state above — this is how visiting `/f/nowak` behaves.
- **Reset on switch:** `ensureFamily` is a no-op when the target family is already current or loading (de-duped via `requestedKey`). Otherwise — except on the very first load of the session — it clears the previous family's state before loading the new one: `familyStore.reset()` (people/unions/focus/error), `selectionStore.reset()` (closes the open person, clears cached detail), and `panelStore.clearPersons()` (closes every rail/popup panel). **The very first load resets nothing**, so a cold deep link straight into `/f/kowalski/person/…` still opens that person's panel instead of having it cleared out from under itself. A `requestToken` counter guards a superseded in-flight request (a fast double-switch) from clobbering the newer one's state when it resolves late. **A failed load is retried:** on error `requestedKey` is cleared, so the next navigation (even to the same family) refetches and, since it is no longer the de-duped "current" request, resets state as any other switch would.
- **Ids are per-family:** a person id like `p-0001` names a different person in each family, so nothing about the panel/selection state is meaningful across a switch — the reset above is required, not just tidiness.

See [app-shell-and-localization.md](app-shell-and-localization.md#family-tree-switcher-familyswitchervue) for the switcher UI that drives this, and [search-and-navigation.md](search-and-navigation.md#routing--deep-links-routerindexts-routerfamilyroutests-viewstreeviewvue) for the route shapes.
