...
- Under the Film theme the top header (`.app-bar`) gets a flat dark graphite band (`#1f2125`) with a `--panel-edge` bottom rule and a soft drop shadow, so the masthead separates from the `#5c5c5c` medallion canvas; Classic leaves the header transparent.
