...
```json
{
  ...
  "families": [
    { "id": "perovsky", "source": "family.json", "name": { "ru": "Перовские" }, "mediaPrefix": null },
...
- `families[].source` is a seed path resolved **relative to the registry file's own location** (same rule as `FamilyData:Source`: an absolute path or a `gs://` URI passes through unchanged; a relative path sits beside the registry, not the working directory).
- `families[].name` (optional) is a `LocalizedTextDto`-shaped display name; `families[].mediaPrefix` (optional) overrides the default `portraits`/`portraits/{familyId}` media-key prefix for that family's seed media.
...
**Person-id rule:** every person in every family's seed must have an id matching `p-<digits>` (e.g. `p-0001`). A seed record that doesn't is **not rejected** — the mismatched people load and appear in listings — but they log once (Warning, then Debug) and the app documents them as **unopenable**: their detail routes are not guaranteed to resolve. Keep this rule when authoring or generating a new family's seed file.
