...
**Map picker ([`MapPicker.vue`](../../../src/frontend/src/components/MapPicker.vue)):** when the build-time `VITE_GOOGLE_MAPS_API_KEY` env var is configured, it shows a debounced (350 ms) free-text city search — up to 5 candidates via the Google Geocoding REST API — and an interactive Google Map (Maps JS) with a draggable pin (default center Minsk, 53.9/27.56, when the row has no coordinates yet). Picking a search result or dragging the pin fills `lat`/`lng`, looks up the locality name in all three locales via the same Geocoding endpoint and fills any locale the picker actually resolved (leaving a locale the editor already typed untouched if the lookup returns nothing for it), and derives a keyless `mapUrl`. **Without a configured key, or if the Maps script fails to load,** the picker degrades to plain manual **latitude/longitude number inputs** (`data-test="manual-lat"`/`manual-lng"`) — local dev and CI run keyless by default, and the manual path still derives the same `mapUrl` from the typed coordinates.
