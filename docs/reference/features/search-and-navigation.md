...
**Dossier ([`MemberDetail.vue`](../../../src/frontend/src/components/MemberDetail.vue)):** fetches `GET /api/people/{id}` fresh on selection (not the summary already in the store) to get full detail. A framed portrait medallion + name + `née …` + lifespan sit beside a **Find on tree** button that navigates to `/person/:slug` and glides the oak camera to that person on arrival. Below, a **field-tablet grid** (given name, surname, maiden name, sex, vocation, **Born** = date + place, **Died** = date + place). A signed-in editor sees an **Edit details** button (`data-test="fields-edit"`) that swaps the read-only tablets for [`MemberFieldsEditor`](../../../src/frontend/src/components/MemberFieldsEditor.vue) — tabbed ru/be/en name inputs, sex/vocation dropdowns, and birth/death-year inputs, with a resilient Save (copies `BiographyEditor`: dirty-gated, buffers kept on failure, confirm-on-discard) that posts to `PUT /api/people/{id}/profile`. **Only changed fields override:** the editor takes the current sparse override as its payload base and submits `override ∪ edits`, so untouched fields keep inheriting the `family.json` seed; a per-field **reset** (`↺`, shown only for a currently-overridden field) clears that field's override (submits `null`). After a successful save the store patches names/sex/vocation in place, refetches the graph only when **birth year** changed (so the oak re-lays-out), and recomputes the `/members/:slug` URL. Then **Biography** and **Residences** side-by-side in double-bordered gilt panels (biography only when non-empty in the current locale; residences as place + optional year span — **read-only**, cut 1c), then the same read-only [photo grid](person-details.md#photo-grid) used elsewhere (`can-edit` forced `false` here).
