...
- A signed-in editor sees three independent edit affordances in the Members dossier — **Edit details** (scalar fields), the biography edit button, and the residences edit button — each opens its own editor and saves independently; an anonymous visitor sees the same dossier with none of the edit buttons rendered (`canEdit` gates them). No add/remove-relative or relationship-editing controls exist yet (cut 2).
