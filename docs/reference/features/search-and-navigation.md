...
**Roster index:** a live substring search box (`data-test="members-search"`, same [`personMatchesQuery`](#search-membersvue-composablesusesearchmatchests) predicate as the tree's nav-bar search) filters the flat people list, sorted by localized surname. Each row shows a thumbnail (or an empty placeholder), full name, and a `birth – death` year span; a footer shows the match count and an empty-state message when nothing matches.

**Dossier ([`MemberDetail.vue`](../../../src/frontend/src/components/MemberDetail.vue)):** fetches `GET /api/people/{id}` fresh on selection (not the summary already in the store) to get full detail. Shows portrait (or an initials fallback), name, maiden name (`née …`), lifespan, a **Find on tree** button that navigates to `/person/:slug` for the same person, sex/vocation fields, biography (only when non-empty in the current locale), residences (place + optional year span), the same read-only [photo grid](person-details.md#photo-grid) used elsewhere (`can-edit` forced `false` here), and an **immediate-family cluster**.

**Immediate-family cluster ([`MemberFamilySheet.vue`](../../../src/frontend/src/components/MemberFamilySheet.vue), [`composables/useRelatives.ts`](../../../src/frontend/src/composables/useRelatives.ts)):** groups the person's **parents, spouse(s), siblings, children** (only non-empty groups render) as clickable chips (portrait thumb + name + year span); clicking a chip selects that relative in the Members page. `deriveRelatives` is pure/side-effect-free: siblings share at least one parent id (half-siblings included, self excluded); spouses/children come from any union that includes the person; each group sorts by birth year then id.
