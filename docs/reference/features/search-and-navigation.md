...
A signed-in editor sees its own edit affordance (`data-test="residences-edit"`) on the **Residences** panel header, independent of the scalar **Edit details** and **Biography** editors — clicking it swaps the read-only list for [`ResidencesEditor.vue`](../../../src/frontend/src/components/ResidencesEditor.vue). Each row holds a place name per locale (`data-test="place-ru-{i}"`/`place-be-{i}`/`place-en-{i}`, at least one non-blank locale required), a `fromYear`/`toYear` span, a 📍 button (`data-test="pick-{i}"`) that opens an inline map picker for that row (only one row's picker is open at a time), and a remove button (`data-test="remove-{i}"`). Rows are added with `data-test="add-residence"`; the client applies **no cap** on adding rows — exceeding the server's 10-row limit surfaces the `PUT` 400 on save, not a disabled Add button.
