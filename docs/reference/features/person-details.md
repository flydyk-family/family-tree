...
- **Header ([`PersonHeader.vue`](../../../src/frontend/src/components/PersonHeader.vue)):** 84×84 circular portrait (video → still → initials, see below; clickable → lightbox), full localized name, optional `née {maidenName}`, formatted lifespan (with `~` for approximate years), vocation icon + label.
