...
- **Header ([`PersonHeader.vue`](../../../src/frontend/src/components/PersonHeader.vue)):** 84×84 circular portrait (video → still → initials, see below; clickable → lightbox), full localized name (**`Given Middle Surname`** — the middle name is a patronymic, RU "Отчество", included when present via `formatPersonName`), optional `née {maidenName}` (**non-male persons only** — hidden for males), formatted lifespan — the **full `день.месяц.год`** when the day and month are known (`01.01.1861–19.03.1916`), else `MM.YYYY` or the year alone, with `~` for approximate dates. Below the lifespan, a row holds the vocation icon + label (when present) and an action group (`.header__actions`, right-aligned via `margin-left: auto` on the group itself, not each button) containing an **Open in members** link (a pill-styled `RouterLink`); the row wraps when it's too narrow for both — the vocation drops above and the action group stays right-aligned on its own line, which is what happens in the narrower rail panel. The link targets `{ name: 'members', params: { slug: personSlug(person) } }` (the bare person id until the family graph has that person) (see [features/search-and-navigation.md](search-and-navigation.md#members-page-readonly-membersslug)) — the reverse of the Members dossier's **Find on tree**. (The tree medallion still shows only the compact year span.) When the person carries `familyLinks` (see [features/backend-api.md](backend-api.md#familydataregistry--the-family-registry)) whose `family` is a **registered** family other than the one currently shown, the action group gets one more pill per link (`data-test="open-family-link"`; filter, label and target come from the shared [`useFamilyLinks`](../../../src/frontend/src/composables/useFamilyLinks.ts) composable, which the Members dossier uses too), grouped alongside **Open in members** at the row's right edge. A link to an unregistered family, or back to the active family, is dropped rather than shown as a dead or redundant button. An `origin` link (the family the person married in from) is labelled `{name} family tree` (`family.openOrigin`), where `{name}` is the linked family's localized name. A `joined` link's label is gendered by the person's `sex`: `Family he joined: {name}` / `Family she joined: {name}`, or the neutral `Family they joined: {name}` otherwise (`family.openJoinedMale` / `family.openJoinedFemale` / `family.openJoined`; ru/be likewise: `Перешёл` / `Перешла` / `Перешёл(ла)`). Every action in the group is a real `<a href>` (`RouterLink`), so middle-click / Ctrl+click opens it in a new tab. Clicking a family link navigates to `/f/<family>/person/<personId>` (or that family's tree root when the link has no `personId`); the default family renders this as the unprefixed `/person/<slug>` / `/`. The counterpart's bare id is a valid slug and is swapped for the friendly one once that family's graph loads. Landing on a person route this way re-arms the arrival glide — like any family switch that lands on a person route, not just these buttons (a browser Back/forward across families or the Family tree switcher does the same) — so the oak camera centers on the linked person once the target family's layout is ready (see [features/search-and-navigation.md](search-and-navigation.md#routing--deep-links-routerindexts-routerfamilyroutests-viewstreeviewvue)); browser Back returns to the tree the visitor came from.
...
**A save that resolves after a family switch is dropped.** Person ids repeat across families, so each editor (`BiographyEditor`, `PersonPhotos`, and the Members dossier's `MemberFieldsEditor` / `ResidencesEditor`) captures `selectionStore.generation` before its API call and skips its `saved` / `updated` emit if a family switch bumped it meanwhile — the host never writes the stale response into the other family's selection cache or roster.
