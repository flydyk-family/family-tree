...
- **Media infra:** R2-backed `/media` Pages Function (range requests), local-dev media plugin, upload script, AI portrait generator script. **Editor photo upload (runtime, in-app):** signed-in editors can upload a portrait or add gallery photos via the photo manager in the bigger-view popup (`POST /api/people/{id}/photos`, `DELETE`, `promote`); bytes go to R2 via the API and persist as overrides in Firestore (in-memory locally); a read-only gallery strip is visible to all visitors. The `scripts/upload-media.mjs` bulk path remains for seed media.
...
- **Portrait `gallery[]`** — field exists on the model and is now surfaced in the UI: a read-only thumbnail strip is visible to all visitors in the bigger-view popup; editors can add/remove/promote gallery photos via the photo manager tab. Seed data has no gallery photos; the strip is hidden when the gallery is empty.
