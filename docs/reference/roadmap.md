...
- **Lock Cloud Run ingress to Cloudflare — network level** — the application-level shared-secret gate (`X-Origin-Verify`) described below has shipped; what remains deferred is **network-level** isolation (GCP Load Balancer + Cloud Armor restricted to Cloudflare IP ranges, or Authenticated Origin Pulls). The application gate is sufficient for the current threat model; network-level hardening is a further follow-up.
