...
- **Production CSP** ([`public/_headers`](../../src/frontend/public/_headers)): same-origin by default (`default-src 'self'`, `connect-src 'self'`, `img-src 'self' data:`, `font-src 'self'`, `frame-ancestors 'none'`, etc.) — hence self-hosted fonts and the Pages-Function proxies for `/api` and `/media`. Third parties are `https://accounts.google.com/gsi/…` (allowed in `script-src`/`frame-src`/`connect-src`/`style-src`, plus `https://*.googleusercontent.com` in `img-src`), required for the Google sign-in flow, and `https://maps.googleapis.com` (allowed in `script-src` for the Maps JS SDK, plus `https://maps.googleapis.com https://maps.gstatic.com https://*.gstatic.com blob:` in `img-src` for map tiles and `worker-src 'self' blob:`), required for the residence map picker's interactive map. `connect-src` also keeps `https://maps.googleapis.com` — the app itself makes no geocoding request to Google (search / reverse / localized names go through our own same-origin `/api/geocode/*` backend proxy), but the Maps JS SDK makes its own runtime requests to `maps.googleapis.com` (viewport/metadata RPCs, telemetry, and vector-tile fetches issued from blob workers, which inherit the document CSP) that `script-src`/`img-src` don't cover.
- **PWA:** [`site.webmanifest`](../../src/frontend/public/site.webmanifest) (`display: standalone`, theme/background `#1b1c1f` — the graphite of the default Film theme, 192/512/maskable icons, `start_url: "/"`); Apple touch icon present.
- **Browser chrome colour is theme-aware:** the `<meta name="theme-color">` tag is kept in sync with the active theme by [`applyTheme.ts`](../../src/frontend/src/styles/applyTheme.ts) — `#1b1c1f` graphite for Film (also the static default in `index.html`), `#faf3df` parchment for Classic — so the mobile address bar / PWA status bar blend with the top of the viewport.
