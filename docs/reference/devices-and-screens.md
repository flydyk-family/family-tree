...
| **Narrow desktop** | `(min-width: 1200px) and (max-width: 1299.98px)` ([`NARROW_DESKTOP_MEDIA_QUERY`](../../src/frontend/src/composables/useMediaQuery.ts)) | Above the mobile switch but tight for an inline search field: the app bar **collapses search to a ⌕ icon** that reveals a full-width search row on click, and **hides the heraldic crest** (tabs stay). Wider desktops show the inline search pill and the crest. |
