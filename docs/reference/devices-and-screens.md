...
- **Production CSP** ([`public/_headers`](../../src/frontend/public/_headers)): same-origin by default (`default-src 'self'`, `connect-src 'self'`, `img-src 'self' data:`, `font-src 'self'`, `frame-ancestors 'none'`, etc.) — hence self-hosted fonts and the Pages-Function proxies for `/api` and `/media`. The one third party is **Google Identity Services**: `script-src` allows `https://accounts.google.com/gsi/client`, `frame-src`/`connect-src` allow `https://accounts.google.com/gsi/`, `style-src` allows `https://accounts.google.com/gsi/style`, and `img-src` allows `https://*.googleusercontent.com` (sign-in avatars). Without these the GIS client script is CSP-blocked and sign-in cannot load.
