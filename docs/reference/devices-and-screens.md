...
- **Production CSP** ([`public/_headers`](../../src/frontend/public/_headers)): same-origin by default (`default-src 'self'`, `connect-src 'self'`, `img-src 'self' data:`, `font-src 'self'`, `frame-ancestors 'none'`, etc.) — hence self-hosted fonts and the Pages-Function proxies for `/api` and `/media`. Third parties are `https://accounts.google.com/gsi/…` (allowed in `script-src`/`frame-src`/`connect-src`/`style-src`, plus `https://*.googleusercontent.com` in `img-src`), required for the Google sign-in flow, and `https://maps.googleapis.com` (allowed in `script-src`/`connect-src`, plus `https://maps.googleapis.com https://maps.gstatic.com https://*.gstatic.com blob:` in `img-src` and `worker-src 'self' blob:`), required for the residence map picker's Maps JS + Geocoding calls.
