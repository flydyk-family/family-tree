...
Applied to all routes (mirrors the API headers) plus a strict **CSP** (`default-src 'self'`, `connect-src 'self'`, `frame-ancestors 'none'`, …), with a narrow allowance for **Google Identity Services** (`https://accounts.google.com/gsi/…` in `script-src`/`frame-src`/`connect-src`/`style-src`, `https://*.googleusercontent.com` in `img-src`) so sign-in loads, and `https://static.cloudflareinsights.com` in `script-src` for the **Cloudflare Web Analytics** beacon that Pages auto-injects (its reporting POST goes to `/cdn-cgi/rum` on our own origin, already covered by `connect-src 'self'`). HSTS includes `preload`.
