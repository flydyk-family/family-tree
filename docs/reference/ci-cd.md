...
| `VITE_GOOGLE_MAPS_API_KEY` | GitHub Actions **variable** (public) | `deploy-spa` build step — baked into the SPA bundle; loads the **Maps JavaScript SDK only** (the interactive map + pin) |
...
> **`VITE_GOOGLE_MAPS_API_KEY` is not provisioned by any script — the owner must create it themselves** in the Google Cloud Console and set it as a GitHub Actions repo variable. It is baked into the public bundle and used **only to load the Maps JavaScript SDK** (the interactive map + pin) — geocoding (search / reverse / localized names) is proxied server-side (below), so this key must be restricted by **HTTP referrer** (the production domain) **and** by **API** (**Maps JavaScript API** only) — an unrestricted key scraped from the bundle is billable by anyone. Until the variable is set, the deploy still succeeds and the map picker degrades to manual lat/lng entry.
>
> **`GoogleMaps__GeocodingApiKey` — the server-side geocoding key.** Unlike `VITE_GOOGLE_MAPS_API_KEY`, this key is **not** a GitHub Actions variable and is **not** referenced by `deploy.yml` — it's a **Cloud Run environment variable**, set once out-of-band exactly like the Firestore/R2 settings above (config key `GoogleMaps:GeocodingApiKey`, bound as `GoogleMapsOptions`). It backs the `CanEdit`-gated `/api/geocode/*` proxy (search / reverse / localized names — see [features/backend-api.md](features/backend-api.md#get-apigeocodesearch-get-apigeocodereverse-get-apigeocodenames)) and is never sent to the browser, so it should be restricted by **API only** (**Geocoding API**) with **no HTTP referrer restriction** — the Geocoding web service rejects referrer-restricted keys, since it expects server-side (IP-restricted, or unrestricted) use. Set it with:
> ```
> gcloud run services update <CLOUD_RUN_SERVICE> --project <PROJECT_ID> --region <REGION> \
>   --update-env-vars GoogleMaps__GeocodingApiKey=<key>
> ```
> Local dev equivalent (same mechanism as the `Authentication:Google:*` secrets — see [`docs/ci-cd/google-signin-setup.md`](../ci-cd/google-signin-setup.md)): `dotnet user-secrets set "GoogleMaps:GeocodingApiKey" "<key>" --project src/backend/FamilyTree.Api`. Absent a configured key, the proxy degrades quietly (empty results, no HTTP call, no error) rather than failing.
...
**Firestore backups** (provisioned by `setup-gcp-deploy.ps1`): **PITR** (continuous 7-day recovery window) plus **managed backup schedules** — daily kept 7 days and weekly (Sunday) kept 14 weeks — protect the durable `personOverrides` (biography edits). Both steps are idempotent. Restores create a **new** database (never overwrite `(default)`); recovery is a supervised operation — runbook in [`deploy.md`](../ci-cd/deploy.md#firestore-backup--restore).
...
Applied to all routes (mirrors the API headers) plus a strict **CSP** (`default-src 'self'`, `connect-src 'self'`, `frame-ancestors 'none'`, …), with a narrow allowance for **Google Identity Services** (`https://accounts.google.com/gsi/…` in `script-src`/`frame-src`/`connect-src`/`style-src`, `https://*.googleusercontent.com` in `img-src`) so sign-in loads. It also allows `https://static.cloudflareinsights.com` in `script-src` for the **Cloudflare Web Analytics** beacon that Pages auto-injects; the beacon's reporting POST goes to `/cdn-cgi/rum` on our own origin, already covered by `connect-src 'self'`. It also allows **Google Maps** (`https://maps.googleapis.com` in `script-src` for the Maps JS SDK, `https://maps.googleapis.com https://maps.gstatic.com https://*.gstatic.com blob:` in `img-src` for map tiles, plus `worker-src 'self' blob:`) for the residence map picker. `connect-src` also keeps `https://maps.googleapis.com` — not for the app's own geocoding calls (those go through our own `/api/geocode/*` backend, already covered by `connect-src 'self'`), but because the Maps JS SDK itself makes runtime requests to `maps.googleapis.com` (viewport/metadata RPCs, telemetry, and vector-tile fetches from blob workers, which inherit the document CSP) that `script-src`/`img-src` don't cover (see [Editing residences](search-and-navigation.md#editing-residences-signed-in-editors-cut-1c) and [backend-api.md](features/backend-api.md#get-apigeocodesearch-get-apigeocodereverse-get-apigeocodenames)). HSTS includes `preload`.

### Health checks
- API: `GET <cloud-run-url>/health` → `{ status, version, commit, degradedFamilies }` (commit from `APP_COMMIT`; `"local"` if unset; `degradedFamilies` lists non-default family trees whose refreshes keep failing, `[]` normally — the deploy check reads only the first three). **Not** proxied through Pages.
