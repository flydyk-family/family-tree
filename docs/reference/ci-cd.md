...
The default `*.pages.dev` host is blocked at the network level in Belarus/Russia (blanket `pages.dev` filtering + DPI dropping Cloudflare's default-on **ECH** / TLS 1.3 handshake). The fix is to serve the app from a custom domain on our own Cloudflare zone (the apex `perovsky.family`) and **disable ECH** on that zone via the Cloudflare API. Full runbook incl. the exact PATCH and how to revert: [`docs/ci-cd/custom-domain-and-ech.md`](../../docs/ci-cd/custom-domain-and-ech.md).
